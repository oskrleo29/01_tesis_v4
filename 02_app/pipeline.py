...
ARTEFACTS_PATH = os.path.join(
    PROJECT_ROOT, "artefactos_modelo_hibrido.pkl"
)
...
# Features del modelo WLS
FEATURES = list(model_wls.feature_names_in_)
MAP_CIUDAD_NORMALIZADA = {
    unicodedata.normalize("NFKD", ciudad)
    .encode("ascii", "ignore")
    .decode("ascii")
    .strip()
    .upper(): info
    for ciudad, info in MAP_CIUDAD.items()
}
...
def _resolver_valor_mapeado(valor, mapping, nombre):
    valor_normalizado = _normalizar_texto(valor)
    for clave, destino in mapping.items():
        if _normalizar_texto(clave) == valor_normalizado:
            return destino
...
def _construir_df_regresion(payload):
    ...
    ciudad_info = MAP_CIUDAD_NORMALIZADA.get(ciudad_normalizada)
    if ciudad_info is None:
    ...
    row = {
        ...
        # ── CAT_VARS (str) ───────────────────────────────
        # Estrato es CAT en el WLS — entrenado con int64
        "Estrato"            : int(estrato),
        "REGION"             : ciudad_info["REGION"],
        "actividad_ppal"     : _resolver_valor_mapeado(
            actividad, MAP_ACTIVIDAD, "actividad_principal"
        ),
        "Sexo_"              : payload["sexo"],
        "nivel_educ_agrupado": _resolver_valor_mapeado(
            nivel_educ, MAP_NIVEL_EDUCATIVO, "nivel_educativo"
        ),
        ...
        "tipo_vivienda_agrup": _resolver_valor_mapeado(
            tipo_viv, MAP_TIPO_VIVIENDA, "tipo_vivienda"
        ),
        "dominio_agrup"      : ciudad_info["dominio_agrup"],
        # flag_pensionado es CAT en el WLS
